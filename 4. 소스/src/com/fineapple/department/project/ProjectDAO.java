...
public class ProjectDAO {
	...
	public ArrayList<ProjectListDTO> getProjectListNow(String departmentSeq) {
		try {
			String sql = "select distinct seq, depseq, respseq, subject, startdate, enddate, goal, type, department, respname, useq"
					+ " from vProjectList where endDate >= sysdate and type = 'project' and depSeq = " + departmentSeq + " order by seq desc";
			Statement stat = conn.createStatement();
			...
			while (rs.next()) {
				...
				if (!compare.equals(rs.getString("seq"))) {	//기존에 읽어온 적 없는 프로젝트
					...
					if (rs.getString("uSeq") != null) {	//uSeq를 발견
						...
						ProjectTotalDTO udto = getTotalUnit(rs.getString("uSeq"));
						...
						if (rs.getString("dSeq") != null) {	//dSeq를 발견
							...
							ProjectTotalDTO ddto = getTotalDetail(rs.getString("dSeq"));
	 ...
	 * @return 해당 단위
	 */
	private ProjectTotalDTO getTotalUnit(String uSeq) {
		try {
	 ...
	 * @return 해당 작업
	 */
	private ProjectTotalDTO getTotalDetail(String dSeq) {
		try {
